...
def test_one_epoch(args, net, test_loader):
    ...
    for i, data in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        ...
        #loss_1 = torch.mean(mask1 * torch.sum((flow_pred - flow) * (flow_pred - flow), -1))
...
def train(args, net, train_loader, test_loader, boardio, textio):
    if args.use_sgd:
        ...
        #print('net', net)
        # 微调 fine-tune
        nn.init.xavier_uniform_(net.conv2.weight);
        print('net param:', list(net.named_parameters()))
        
        params_1x = [param for name, param in net.named_parameters()
                     if name not in ["conv2.weight", "conv2.bias"]]
    ...
    for epoch in range(args.epochs):
        ...
        if best_test_loss >= test_loss:
            ...
            if torch.cuda.device_count() > 1:
                torch.save(net.module.state_dict(), 'checkpoints/%s/models/model.best_n.t7' % args.exp_name)
            else:
                torch.save(net.state_dict(), 'checkpoints/%s/models/model.sample.t7' % args.exp_name)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='SceneflowDataset',
                        ...
                        help='dataset to use')
    parser.add_argument('--dataset_path', type=str, default='/home/cui/data/kitti_rm_ground/', metavar='N',
                        help='dataset to use')
    parser.add_argument('--model_path', type=str, default='/home/cui/workspace/deepLearning/learning3d/pretrained/exp_flownet/models/model.best.t7', metavar='N',
    ...
    if args.model == 'flownet':
        net = FlowNet3D().cuda()
        #net.apply(weights_init) # 全部conv2d 权重初始化
        if args.pretrained:
